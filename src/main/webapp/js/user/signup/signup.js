document.addEventListener('DOMContentLoaded', function() {
	...
	document.getElementById('id').addEventListener('input', function() {
		...
		activateButton(document.getElementById('isIdDuplicated'));
	});
	...
	document.querySelector('input[name="nickname"]').addEventListener('input', function() {
		...
		activateButton(document.getElementById('isNicknameDuplicated'));
	});
	...
	document.querySelector('input[name="email"]').addEventListener('input', function() {
		...
		activateButton(document.getElementById('isEmailDuplicated'));
	});
		...
		$.ajax({
			...
			success: function(response) {
				if (response === 'duplicated') {
					...
					alert('사용 가능한 아이디입니다.');
					idChecked = true;
					deactivateButton(document.getElementById('isIdDuplicated'));
				}
			},
			error: function() {
				alert("전송 오류");
...
function isNicknameDuplicated() {
	...
	if (!nicknameRegex.test(nickname)) {
		alert("닉네임은 1자 이상 30자 이하의 영문, 숫자, 마침표, 언더바만 입력 가능합니다.");
		...
		$.ajax({
			...
			success: function(response) {
				if (response === 'duplicated') {
					...
					deactivateButton(document.getElementById('isNicknameDuplicated'));
				}
			},
			error: function() {
				alert("전송 오류");
		...
		$.ajax({
			...
			success: function(response) {
				if (response === 'duplicated') {
					...
					alert('사용 가능한 이메일입니다.');
					emailChecked = true;
					deactivateButton(document.getElementById('isEmailDuplicated'));
				}
			},
			error: function() {
				alert("전송 오류");
